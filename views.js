...
`
                 <label>DNS:</label>
                 <input type="url" name="dns" value="${query.dns || ''}" placeholder="Ex: http://amigosunidos.lol:80" required>
                 
                 <label>Username:</label>
                 <input type="text" name="username" value="${query.username || ''}" required>
                 
                 <label>Password:</label>
                 <input type="password" name="password" value="${query.password || ''}" required>
...
class="advanced-settings-header" onclick="toggleAdvancedSettings()">
                      ...
                      <div class="advanced-settings-content" id="advanced-settings-content">
                         ...
                         <label>URL do Proxy:</label>
                           <input
type="url" name="proxy" value="${query.proxy || ''}">
...
color: #ccc;">
               ${showConfigFields && showSupportSection ? ` <p>Addon criado com paixão por McCoy88f - <a href="https://github.com/mccoy88f/OMG-Premium-TV" target="_blank">GitHub Repository</a></p>
               <p>Addon editado por LAppDesign.</p> <h3 style="margin-top: 20px;">Apoia este projeto!</h3>


               <div style="margin-top: 15px;">
                   <a href="https://www.buymeacoffee.com/mccoy88f"
target="_blank">
...
<div id="toast" class="toast">URL Copiado!</div>
           ...
           <script>
               ...
               function updateConfig(event) {
                   ...
                   const dns = form.dns.value;
                   const username = form.username.value;
                   const password = form.password.value;
                   const epgEnabled = form.epg_enabled.checked;

                   const m3uUrl = `${dns}/playlist/${username}/${password}/m3u_plus`;
                   const epgUrl = `${dns}/xmltv.php?username=${username}&password=${password}`;
                   
                   const newQuery = new URLSearchParams();
                   newQuery.set('m3u', m3uUrl);
                   newQuery.set('epg', epgUrl);
                   if (epgEnabled) {
                       newQuery.set('epg_enabled', 'true');
                   }
                   
                   // Adicionar outros campos que precisam ser passados para a URL
                   const advancedSettings = document.getElementById('advanced-settings-content');
                   if (advancedSettings) {
                       const advancedInputs = advancedSettings.querySelectorAll('input');
                       advancedInputs.forEach(input => {
                           if (input.type === 'checkbox' && input.checked) {
                               newQuery.set(input.name, 'true');
                           } else if (input.type !== 'checkbox' && input.value) {
                               newQuery.set(input.name, input.value);
                           }
                       });
                   }

                   window.location.href = `?${newQuery.toString()}`;
               }
           </script>
       </div>
