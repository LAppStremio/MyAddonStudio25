...
// Rota principal para a página de configuração. AGORA COM VERIFICAÇÃO.
app.get('/', async (req, res) => {
    const protocol = req.headers['x-forwarded-proto'] || req.protocol;
    const host = req.headers['x-forwarded-host'] || req.get('host');
    
    // Pega o código de acesso da URL
    const accessCode = req.query.access_code;
    const correctCode = process.env.ACCESS_CODE;

    // Se o código não existir ou for incorreto, redireciona para a página principal.
    // Esta é a verificação que força o login.
    if (!accessCode || accessCode !== correctCode) {
        // Renderiza a página com o formulário de código de acesso, sem o conteúdo completo.
        // A lógica de views.js irá tratar disso, pois contentVisible será falso.
        return res.send(renderConfigPage(protocol, host, { access_code: null }, config.manifest));
    }

    // Se o código estiver correto, renderiza a página de configuração completa.
...
// Rota para o manifest - sistema padrão
app.get('/manifest.json', async (req, res) => {
    try {
        ...
        const configUrl = `${protocol}://${host}/?access_code=${process.env.ACCESS_CODE}`;
...
app.post('/api/resolver', async (req, res) => {
    ...
    try {
        if (action === 'download' && url) {
            ...
            res.json({ 
                success: true, 
                message: stopped ? 'Aggiornamento automatico fermato' : 'Nessun aggiornamento pianificado da fermar' 
            });
        } else {
...
async function startAddon() {
   ...
   try {
       ...
       app.listen(port, () => {
          ...
          console.log('Link alla pagina de configuracao:', `http://localhost:${port}`);
          console.log('=============================\n');
        });
   } catch (error) {
