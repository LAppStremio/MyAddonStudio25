...
const getViewScripts = (protocol, host) => {
   return `
       ...
       function getConfigQueryString() {
           ...
           // Adicionando os novos campos diretamente para a URL
           const dns = form.querySelector('input[name="dns"]')?.value;
           const username = form.querySelector('input[name="username"]')?.value;
           const password = form.querySelector('input[name="password"]')?.value;
           const epgEnabled = form.querySelector('input[name="epg_enabled"]')?.checked;
           
           if (dns && username && password) {
               const m3uUrl = \`\${dns}/playlist/\${username}/\${password}/m3u_plus\`;
               const epgUrl = \`\${dns}/xmltv.php?username=\${username}&password=\${password}\`;
               
               params.append('m3u', m3uUrl);
               params.append('epg', epgUrl);
           }
           
           // Adicionar os outros parâmetros
           const inputs = form.querySelectorAll('input:not([name="dns"]):not([name="username"]):not([name="password"])');
       ...
       function updateConfig(e) {
           e.preventDefault();
           const configQueryString = getConfigQueryString();
           window.location.href = \`${protocol}://${host}/?${configQueryString}\`;
       }
       ...
       async function scheduleResolverUpdates() {
           ...
           try {
               ...
               // Cria um campo oculto no formulário se não existir
               let hiddenField = document.querySelector('input[name="resolver_update_interval"]');
               if (!hiddenField) {
                   hiddenField = document.createElement('input');
                   hiddenField.type = 'hidden';
               ...
               // Define o valor do intervalo no campo oculto
