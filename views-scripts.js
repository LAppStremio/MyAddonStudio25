...
const getViewScripts = (protocol, host) => {
   return `
       ...
       function getConfigQueryString() {
           ...
           // Lógica para obter os novos campos
           ...
           if (dns && username && password) {
               // Constrói os URLs M3U e EPG com base nos novos campos
               const m3uUrl = \`\${dns}/playlist/\${username}/\${password}/m3u_plus\`;
               const epgUrl = \`\${dns}/xmltv.php?username=\${username}&password=\${password}\`;

               // Adiciona os URLs dinamicamente
               params.append('m3u', m3uUrl);
               params.append('epg', epgUrl);
           }

           // Adiciona os outros parâmetros do formulário, exceto os que foram substituídos
           formData.forEach((value, key) => {
               if (key !== 'dns' && key !== 'username' && key !== 'password' && key !== 'm3u' && key !== 'epg') {
